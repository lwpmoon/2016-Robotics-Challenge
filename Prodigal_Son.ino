...
int temp = 0; //Leave this here because I forgot what I was doing with it...
const int accuracy = 15;//Sets a window (+/-) of accuracy with navigation
const int range = 1; //Sets range for Ping
const int rate = 200; //Sets ping refresh rate
const int SPEED = 75; //Sets speed of the motors
...
void beaconMissing()
{
	...
	Serial.println("No beacon signal in range...");
	//lcd.setCursor(0, 1);
	//lcd.print("in range...");
	delay(1000);
}
...
void loop() {
	//Check if the beacon is transmitting...
	scout.updatePresent();
	while (scout.returnPresent())beaconMissing();

	//lcd.clear();
	googlyEyes.updateDistance();
	while (googlyEyes.distanceCM() > range) {
		if (scout.returnPresent() != true) {
			//get calibration for bno
			//navigator.getCalibration();
			//Where am I pointed?
			navigator.updateHeading();
			Serial.print("Corrected heading: ");
			Serial.println(navigator.returnHeading());

			//Make important decisions...
			if (!groundEye.isGroundPres()){
				//lcd.clear();
